...
\begin{proof}\leavevmode
	\begin{enumerate}
		...
		\begin{exer}\label{exer:img-fechada}
			Suponha $E\to F$, $\delta>0$ e $\|Sx\|\geq \delta\|x\|$ (continuidade da inversa?), então $\img(S)$ é fechado.
		\end{exer}
		Suponha que existe uma sequência $(x_n)\subseteq\partial B_Y$ tal que
		\[\|Tx_n-\lambda x_n\|\to 0.\]
		Como $T$ é compacto, podemos supor que $(T(x_n))$ é Cauchy. Como $\lambda\neq0$, $(x_n)$ é Cauchy. Defina
		\[x=\lim_nx_n.\]
		Portanto, $Tx=\lambda x$, e assim $x\in \ker(T-\lambda)=Z$.
		
		Por outro lado, como $(x_n)\subseteq Y$ e $Y$ fechado, $x\in Y$. Assim, $x=0$, absurdo pois a sequência tá contida em $\partial B_Y$.
